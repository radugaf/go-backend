...
proto:
	rm -f protos/*.go
	protoc --proto_path=protos --go_out=pb --go_opt=paths=source_relative --go-grpc_out=pb --go-grpc_opt=paths=source_relative protos/*.proto

evans:
	evans --host localhost --port 9090 -r repl
	
.PHONY: postgres createdb dropdb migrateup migrateuplast migratedown migratedownlast sqlc server mock test evans
